...
/* USER CODE BEGIN PV */
ring_buffer uart2_buffer;    /* uart2 的环形 buffer */
...
int main(void)
{
  ...
  /* USER CODE BEGIN Init */
	...
	USART2_Lock_Init();                 /* 初始化 uart2 的互斥锁 */
 ...
 * 返回值  ： 无
 * 函数功能： USART2 的中断处理函数，
 *            如果发生 RXNE 中断，则读取 ESP8266 接收到的数据，放入 USART2 的环形缓冲区 uart2_buffer
*/
void USART2_IRQHandler(void)
{
	...
	*/	
    ...
    if (((huart2.Instance->SR & USART_SR_RXNE) != RESET) && ((cr1its & USART_CR1_RXNEIE) != RESET))
    {
		...
		*/
		...
		platform_mutex_unlock(&uart2_recv_mutex);  // 释放互斥量，通知某些任务 USART2 接收到了数据，环形缓冲区有数据可读
      	return;
    }
}

/* 从 USART2 的环形缓冲区 uart2_buffer 中读走 1 个字符，放入 c 地址，没有数据可读就阻塞 */
void USART2_Read(char *c, int timeout)
{
	while (1)
	{
