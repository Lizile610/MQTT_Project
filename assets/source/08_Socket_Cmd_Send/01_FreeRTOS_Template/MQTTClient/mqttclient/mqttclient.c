...
static int32_t mqtt_read_buf_malloc(mqtt_client_t* c, uint32_t size)
{
...
static int32_t mqtt_write_buf_malloc(mqtt_client_t* c, uint32_t size)
{
    MQTT_ROBUSTNESS_CHECK(c, 0);
...
mqtt_client_t *mqtt_lease(void)
{
    ...
    c = (mqtt_client_t *)platform_memory_alloc(sizeof(mqtt_client_t));  // 分配结构体
    if (NULL == c)
        return NULL;

    memset(c, 0, sizeof(mqtt_client_t));  // 将指针 c 指向的结构体中每个字节设置为 0 
