...
static char *g_cur_cmd;					   // 全局buffer，指向当前发送出去的命令
 ...
 * 输出参数： 无
 * 返回值  ： 1 - 获得了 "AT+CIPSEND" 命令的响应
 * 	          0 - 没有获得
 * 函数功能： 判断是否可以开始向服务器发送数据
 */
static int GetCIPSENDResp(char *buf)
{
	/* 如果当前命令确实是 "AI+CIPSEND"，并且紧接着收到了 '>'，说明可以开始发送数据 */
...
static void ProcessSpecialATString(char *buf)
{
	...
	while (1)
	{
		...
		while (i < len)
		{
			...
			ring_buffer_write(buf[i], &packet_buffer);
...
int ATReadData(unsigned char *c, int timeout)
{
	...
	do {
		...
		if ( 0 == ring_buffer_read((unsigned char *)c, &packet_buffer) )
			return AT_OK;  // 表示读取成功
		else
		{
...
{	
	...
	while (1)
	{
		...
		else if (GetCIPSENDResp(buf))  /* 判断是否是否可以开始向服务器发送数据 */
		{
			SetATStatus(AT_ERR);					// 设置 AT 响应的状态为 ERROR
			platform_mutex_unlock(&at_resp_mutex);	
...
void Task_ATTest(void *params)
{
	...
	while (1)
	{
		ret = ATSendCmd("AT", NULL, 0, 1000);  // 发送命令 "AT"，不保存响应数据，仅测试响应的状态，阻塞 1000 ms
