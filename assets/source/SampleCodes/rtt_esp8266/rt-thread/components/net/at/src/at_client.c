...
int at_obj_exec_cmd(at_client_t client, at_response_t resp, const char *cmd_expr, ...)
{
    ...
    rt_mutex_take(client->lock, RT_WAITING_FOREVER);  // 获得互斥量，与其它发送 AT 命令的线程互斥
    ...
    if (resp != RT_NULL)
    {
        if (rt_sem_take(client->resp_notice, resp->timeout) != RT_EOK)  /* 等待 AT 命令响应，即等待信号量 resp_notice */
        {
...
static int at_recv_readline(at_client_t client)
{
    ...
    while (1)
    {
        at_client_getchar(client, &ch, RT_WAITING_FOREVER);  // 永远等待，直到得到一个字符

        if (read_len < client->recv_bufsz)
        {
            client->recv_line_buf[read_len++] = ch;  // 保存读到的数据到一个 buffer 里
		 ...
		 * 根据以下条件之一判断接收到了一整行数据，然后返回：
		 *     1. 收到了回车换行
		 *     2. 收到一个特殊字符 client->end_sign 
		 *     3. 收到一个 urc 对象
		 */
    ...
    while(1)
    {
        ...
        if (at_recv_readline(client) > 0)
        {
            ...
            else if (client->resp != RT_NULL)
            {
                ...
                client->resp = RT_NULL;
                rt_sem_release(client->resp_notice);  // 释放信号量 resp_notice
            }
            else
            {
