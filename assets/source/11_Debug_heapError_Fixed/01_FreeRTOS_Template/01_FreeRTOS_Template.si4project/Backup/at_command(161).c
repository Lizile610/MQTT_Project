...
static platform_mutex_t at_resp_mutex;     // 等待 AT 命令响应的信号量
static platform_mutex_t at_packet_mutex;   // 等待网络数据包的信号量
 ...
 * 输入参数： buf -> 暂存 AT 命令响应信息的缓冲区
 * 输出参数： 无
 * 返回值  ： 1 - 在 AT 命令的响应信息中找到了特殊字符串
 * 	          0 - 未找到特殊字符串
 * 函数功能： 在 AT 命令的响应信息中查找是否存在特殊字符串
 *--------------------------------------
*/
...
{	
	char buf[AT_RESP_LEN];  // 从 ESP8266 模块读到的数据会保存在这个 buffer
	int i = 0;
	...
	while (1)
	{
		...
		if (i && (buf[i-1] == '\r') && (buf[i] == '\n'))
		{
			...
			else if (GetSpecialATString(buf))   /* 判断字符串中是否含有特殊的 AT 符，如 "+IPD," */
			{
				...
				i = -1;  // 如果解析失败，下一次循环让 buf 重新从头接收数据
			}
			i++;
		}
	}
