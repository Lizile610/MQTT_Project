 ...
 * 输出参数： 无
 * 返回值  ： 无
 * 函数功能： 使能 USART2 的接收中断
*/
void EnableUART2IRQ(void)
{
    HAL_NVIC_SetPriority(USART2_IRQn, 0, 1);    // 设置USART2中断的抢占优先级为 0，子优先级为 1
    HAL_NVIC_EnableIRQ(USART2_IRQn);            // 使能USART2的中断

    __HAL_UART_ENABLE_IT(&huart2, UART_IT_RXNE);   // 使能USRAT2的接收中断
}
